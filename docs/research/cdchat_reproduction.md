...
The script pins Hugging Face revision `bf08270f943114eee92c5fcd93daf5009d460af4`, performs a small Hugging Face proxy preflight, downloads into `*.part` files with resumable `aria2c` when available, and promotes files to their final checkpoint paths only after size and SHA-256 checks pass. Set `CDCHAT_DOWNLOADER=wget` to force the fallback downloader. It then runs:
...
- On 2026-07-05 UTC, `127.0.0.1:7890` can reach Hugging Face again, but large shard throughput is still slow. Resume the two large weight files as long-running downloads and verify SHA-256 before inference.
